          ...
          <mu-stepper :active-step="vactiveStep" orientation="vertical">
            <mu-step>
              ...
              <mu-step-content>
                <mu-row gutter>
                  ...
                  <mu-col><mu-text-field v-model="fromloc" placeholder="请输入库位"></mu-text-field></mu-col>
                </mu-row>
                <mu-row gutter>
            ...
            <mu-step>
              ...
              <mu-step-content>
                ...
                <mu-button class="step-button" @click="adjust" color="primary">完成</mu-button>
                <mu-button flat class="step-button" @click="vhandlePrev">上一步</mu-button>
              </mu-step-content>
            </mu-step>
          </mu-stepper>
...
<script>
  ...
  import {validateFromLoc, getInvData, adjust} from '@/api/inventory'
  import Vue from 'vue'
  ...
  export default {
    ...
    methods: {
      ...
      adjust() {
        adjust({fromloc:this.fromloc, sku:this.sku, qty:Number(this.qty), storerkey:this.invData.storerkey, lot:this.invData.lot, id:this.invData.id}).then(res => {
          console.log(res.data)
          if(res.data.errFlag === 0){
            this.fromloc = ''
            this.sku = ''
